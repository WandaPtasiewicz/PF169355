...
class MovieRentalShop:
    ...
    def find_movies_by_director(self, movie_director):
        find_movies = [movie for movie in self.movies if movie.director == movie_director]
        if not find_movies:
            raise ValueError ("Movie not found")
        return find_movies

    def find_movies_by_genre(self, movie_genre):
        find_movies = [movie for movie in self.movies if movie.genre == movie_genre]
        if not find_movies:
            raise ValueError ("Movie not found")
        return find_movies

    def find_movies_by_release_year(self, movie_release_year):
        find_movies = [movie for movie in self.movies if movie.release_year == movie_release_year]
        if not find_movies:
            raise ValueError ("Movie not found")
        return find_movies

    def find_movies_by_age_limit(self, movie_age_limit):
        find_movies = [movie for movie in self.movies if movie.age_limit <= movie_age_limit]
        if not find_movies:
            raise ValueError ("Movie not found")
        return find_movies
    ...
    def find_users_by_first_name(self, first_name):
        find_users = [user for user in self.users if user.first_name == first_name]
        if not find_users:
            raise ValueError ("User not found")
        return find_users

    def find_users_by_last_name(self, last_name):
        find_users = [user for user in self.users if user.last_name == last_name]
        if not find_users:
            raise ValueError ("User not found")
        return find_users

    def find_users_by_birth(self, birth):
        find_users = [user for user in self.users if user.birth == birth]
        if not find_users:
            raise ValueError ("User not found")
        return find_users

    def find_active_users(self):
        find_users = [user for user in self.users if user.active == True]
        if not find_users:
            raise ValueError ("User not found")
        return find_users

    def find_inactive_users(self):
        find_users = [user for user in self.users if user.active == False]
        if not find_users:
            raise ValueError ("User not found")
        return find_users
